...
public class PluginInstallerService implements IDynamicService {

    private AgentNodeProperties agentNodeProperties = new AgentNodeProperties();
    private static final IADLogger logger = ADLoggerFactory.getLogger((String)"com.singularity.ee.service.pluginInstaller.VersionSelfMonitoringService");
    ...
    public PluginInstallerService() {
        logger.info("Initializing Agent Plugin Installer Service");
    }

    public PluginInstallerService(AgentNodeProperties agentNodeProperties, long taskInitialDelay, long taskInterval) {
        this();
    ...
    public void start() throws ServiceStartException {
        ...
        if(!agentNodeProperties.isEnabled()) {
            logger.info("Service " + this.getName() + " is not enabled.  So not starting this service.  To start it enable the node property agent.isdk.installer.enabled");
            return;
        }
        ...
        if (this.scheduler == null) {
            throw new ServiceStartException("Scheduler is not set, so unable to start the agent plugin installer service");
        }
        if (this.serviceComponent == null) {
            throw new ServiceStartException("Dagger not initialised, so cannot start the agent plugin installer service");
        }
    ...
    private IAgentRunnable createTask(ServiceComponent serviceComponent) {
        logger.info("Creating Task for agent plugin installer monitoring");
        return new CheckForAgentInstallPluginsTask( this, this.agentNodeProperties, serviceComponent, iServiceContext);
    }
    ...
    public void hotDisable() {
        logger.info("Disabling agent plugin installer service");
        try {
            this.stop();
        }
    ...
    public void hotEnable() {
        logger.info("Enabling agent plugin installer service");
        try {
            this.start();
        }
